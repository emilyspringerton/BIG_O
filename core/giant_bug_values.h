#ifndef BIGO_GIANT_BUG_VALUES_H
#define BIGO_GIANT_BUG_VALUES_H

/* giant_bug_values.h -- Giant Zombie Bug's real value system. Brought back from SHANKPIT
 * (EMILY/BACKLOG.md SECTION 536, "bring SHANKPIT stuff back into BIG_O" follow-up) -- built there
 * natively, forward of the original BIG_O engine merge, founder real-time, 2026-09-22: "add giant
 * zombie bugs (feral AI units) they need a totally unique value system vector based deliberately
 * non human 64 layer hand written llm" -> "use parena". See PARENA/stdlib/big_o/
 * giant_bug_brain.prn's own doc comment for the full, honest "64 layer hand written llm" scope
 * account -- this header is the C-side state/tick half, matching zombie_values.h/humanness.c's
 * own established "stateful stuff stays host C, pure decision formulas move to PARENA" split.
 *
 ...
 * lab_sim.c's own real cloning-facility logic) and their brain is a bio-engineered pass-through of
 * the giant_bug_brain.prn network, not a literal separate neural-interface subsystem. Actually
 * spawning one FROM the lab stays the same named, honest, unstarted gap this repo already names
 * (§9's own "not yet wired into BP_APP_LAB's UI") -- "the lab" has zero UI/interaction model.
 *
 ...
 *
 * REAL, HONEST GAP, found during this port: no live consumer yet. BIG_O's live day/apps/server
 * ServerNpc array is PC_NPC_MAX=8, already fully populated (3 citizens, 1 The Men, 4 zombies,
 * server_spawn_npcs) -- there's no free slot for a 4th role without growing that array, and no
 * spawn/visual-reuse/"Men hold the key" gate wiring exists yet either. Same "correct primitive, no
 * consumer" pattern this repo's own walkie-talkie reverse port (§12) already established -- this
 * is phase 2 of that same effort. */
