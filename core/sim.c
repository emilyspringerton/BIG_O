...
const char *sim_ws_name(int s) { return (s >= 0 && s <= 5) ? WS[s] : "?"; }
const char *sim_band_name(int b) { return (b >= 0 && b <= 3) ? BAND[b] : "?"; }
const char *sim_zone_name(int z) { return (z >= 0 && z <= 4) ? ZONE[z] : "?"; }
const char *sim_costume_name(int c) { return (c >= 0 && c <= 3) ? COST[c] : "?"; }

static int roll100(Sim *s) { /* xorshift32 -> pre-rolled 0..99 handed to the pure rules */
...
void sim_init(Sim *s, uint32_t seed, int nplayers, FILE *out) {
    ...
    if (nplayers > SIM_MAX_PLAYERS) nplayers = SIM_MAX_PLAYERS;
    s->nplayers = nplayers;
...
int sim_add_npc(Sim *s, int zone, int vigilance, int arrogance) {
    if (s->nnpcs >= SIM_MAX_NPCS || zone < 0 || zone > 4) return -1;
    SimNpc *n = &s->n[s->nnpcs];
    n->zone = zone; n->vigilance = vigilance; n->arrogance = arrogance; n->state = WS_UNAWARE; n->accomplice = 0; n->witnessed_event = 0;
    return s->nnpcs++;
}

static int valid_player(const Sim *s, int pl) { return pl >= 0 && pl < s->nplayers && s->p[pl].present && !s->p[pl].cancelled; }

static void apply_decorum(Sim *s, int pl, int action, const char *why) {
    SimPlayer *p = &s->p[pl];
...
static int anyone_notices(Sim *s, int zone, int conspicuous) {
    int seen = 0;
...
int sim_enter(Sim *s, int pl, int zone) {
    ...
    SimPlayer *p = &s->p[pl];
    p->zone = zone;
    fprintf(s->out, "EVENT tick=%d enter p%d %s costume=%s\n", s->tick, pl, sim_zone_name(zone), sim_costume_name(p->costume));
    return sim_observe(s, pl);
}

int sim_observe(Sim *s, int pl) {
...
int sim_release(Sim *s, int pl, int tier) {
    if (!valid_player(s, pl)) return -1;
    ...
    fprintf(s->out, "EVENT #%d tick=%d release p%d tier=%d zone=%s witnesses=%d accomplices=%d together=%d\n",
            s->event_id, s->tick, pl, tier, sim_zone_name(z), count, acc, together);
    int silencing = 0;
    for (int i = 0; i < s->nnpcs; i++) {
        SimNpc *n = &s->n[i];
...
void sim_print_state(const Sim *s, FILE *out) {
    ...
    for (int i = 0; i < s->nplayers; i++) {
        const SimPlayer *p = &s->p[i];
        fprintf(out, "  p%d zone=%s costume=%s token=%d gear=%d decorum=%d band=%s hunted=%d cancelled=%d\n", i,
                sim_zone_name(p->zone), sim_costume_name(p->costume), p->token, p->gear, p->decorum,
                sim_band_name(decorum_band(p->decorum)), p->hunted, p->cancelled);
    }
    for (int i = 0; i < s->nnpcs; i++) {
        const SimNpc *n = &s->n[i];
        fprintf(out, "  npc%d zone=%s vig=%d arr=%d state=%s accomplice=%d\n", i, sim_zone_name(n->zone), n->vigilance, n->arrogance, sim_ws_name(n->state), n->accomplice);
    }
}
