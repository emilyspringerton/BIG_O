...
static float humanness_rand_gaussian(void) {
    ...
    /* Literal, not M_PI -- BIG_O's own strict -std=c99 build (BUILD.bazel) doesn't expose glibc's
       M_PI without a feature-test macro (found live: "M_PI undeclared" under that exact toolchain,
       SHANKPIT's own looser Makefile build never hit this). Matches core/zombie_values.c's own
       already-established convention for the same reason. */
