// zombie_values.c -- see zombie_values.h.
...
void zombie_tick(ZombieState *z, uint32_t now_ms, int has_target) {
    /* Real, fixed-ish per-call step -- callers driving this every real tick (matching
       humanness_tick_mood's own "per tick or per decision cycle" contract) get a real, bounded
       dt via mood_change_at_ms bookkeeping rather than an explicit dt parameter, same v0
       simplification humanness.h's own HumannessState already documents for its single shared
       mood timer. Approximated here as a flat ~1 real second per zombie_tick call -- callers on a
       faster tick should call this less often than every physics tick, not every frame. */
    ...
    /* Frenzy is only ever entered as a real spike (aggression crossing a high threshold while
       already HUNTING, or zombie_get_agitated's own explicit trigger) -- never a plain drift
       target here, matching this header's own "spike, not a slow climb" contract. A FRENZIED
       zombie decays back to HUNTING once aggression drops back under the threshold. */
...
uint32_t zombie_reaction_delay_ms(const ZombieState *z, uint32_t base_ms) {
    ...
    /* Spread scales off base_ms directly, NOT off the mood-adjusted mean -- DORMANT's much larger
       ...
       contract this function documents (found live via zombie_values_test.c's own variance
       assertion failing before this fix). */
...
int zombie_effective_alertness(const ZombieState *z) {
    float base;
    switch (z->mood) {
        case ZOMBIE_MOOD_DORMANT:  base = 10.0f; break;
        case ZOMBIE_MOOD_AGITATED: base = 40.0f; break;
        case ZOMBIE_MOOD_HUNTING:  base = 70.0f; break;
        case ZOMBIE_MOOD_FRENZIED: default: base = 95.0f; break;
    }
    float v = base + z->hunger * 10.0f + z->aggression * 10.0f;
    if (v < 0.0f) v = 0.0f;
