...
/* GSEQ_MAX_CHANNELS -- real, live-verified bug fix, carried over from the upstream fix in
 * SHANKPIT (packages/goldenband/gseq.h, commit d14c7a6): the OLD 256 cap silently failed to load
 * mannequin_npc's own real 65-joint/455-channel clips (gband_skel_npc.c's own 7 channels/joint
 * convention overflows a 256 cap past 36 joints) -- confirmed live testing THIS vendored copy
 * directly (gband_skel_npc_load_kit against day/assets/goldenband's own real mannequin_npc +
 * UAL1_Standard_* clips, not assumed working just because the source vendored cleanly). Bumped to
 * 1024, matching the upstream fix exactly, for the same real headroom margin
 * (GSKEL_MAX_JOINTS(128) * 7 = 896). */
