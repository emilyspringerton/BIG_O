/* bigo_skycfg.h -- the configurable half of the BIG_O skybox: palettes, celestial bodies, clouds, and one profile per
 * weather. Pure C (no GL) so it is unit-tested and loadable on any host. bigo_sky.h renders from a BigoSkyConfig, and the
 * weather (0 CLEAR 1 OVERCAST 2 RAIN 3 STORM) selects a profile, so the sky changes with the weather and a mod maker
 * can restyle everything by editing a text file (see day/assets/skybox_default.cfg for every key).
 *
 * File format: one `key v1 [v2 v3]` per line; `#` starts a comment; keys are case-insensitive. Unknown keys and bad
 * values are errors (reported with the line number), never silently ignored. Colours are RGB 0..1. */
#ifndef BIGO_SKYCFG_H
#define BIGO_SKYCFG_H
#include <ctype.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

typedef struct { float zenith[3], horizon[3]; } BscPalette;
...
typedef struct {
    BscPalette day, golden, dawn, twilight, night;
    ...
    float star_count, star_brightness;  /* <= BSKY_STARS; brightness multiplier */
    float cloud_count, cloud_scale, cloud_speed; /* <= BSKY_CLOUDS; puff size multiplier; drift multiplier */
    ...
    BscWeather weather[4];
} BigoSkyConfig;

static inline void bsc_set3(float *d, float a, float b, float c) { d[0] = a; d[1] = b; d[2] = c; }

static inline void bigo_skycfg_defaults(BigoSkyConfig *c) {
    memset(c, 0, sizeof(*c));
    bsc_set3(c->day.zenith, 0.13f, 0.36f, 0.78f);      bsc_set3(c->day.horizon, 0.60f, 0.79f, 0.95f);
    bsc_set3(c->golden.zenith, 0.20f, 0.30f, 0.58f);   bsc_set3(c->golden.horizon, 1.00f, 0.55f, 0.28f);
    bsc_set3(c->dawn.zenith, 0.24f, 0.30f, 0.60f);     bsc_set3(c->dawn.horizon, 1.00f, 0.62f, 0.50f);
    bsc_set3(c->twilight.zenith, 0.06f, 0.07f, 0.24f); bsc_set3(c->twilight.horizon, 0.42f, 0.24f, 0.36f);
    bsc_set3(c->night.zenith, 0.008f, 0.014f, 0.055f); bsc_set3(c->night.horizon, 0.03f, 0.05f, 0.13f);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        bsc_set3(w->grey_tone, 0.98f, 1.00f, 1.08f);
    }
}

typedef struct { const char *key; float *ptr; int n; } BscKey;

/* All settable keys in one table (also the documentation of what exists). Returns the count written to `out`. */
static inline int bsc_keys(BigoSkyConfig *c, BscKey *out, int max) {
    int n = 0;
...
static inline int bigo_skycfg_parse(BigoSkyConfig *c, const char *text, char *err, size_t errsz) {
    BscKey keys[160]; int nk = bsc_keys(c, keys, 160), applied = 0, line = 0;
    const char *p = text;
    while (*p) {
...
static inline void bsc_clamp1(float *f, float lo, float hi) { if (*f < lo) *f = lo; else if (*f > hi) *f = hi; }

/* Clamp everything into ranges the renderer can trust (colours 0..1, counts within the renderer's arrays). */
static inline void bigo_skycfg_sanitize(BigoSkyConfig *c, int max_stars, int max_clouds) {
    BscKey keys[160]; int nk = bsc_keys(c, keys, 160);
    ...
    for (unsigned i = 0; i < sizeof(cols) / sizeof(cols[0]); i++) for (int v = 0; v < 3; v++) bsc_clamp1(&cols[i][v], 0, 1);
    for (int i = 0; i < 4; i++) {
        BscWeather *w = &c->weather[i];
        float *u[] = { &w->cover, &w->rain, &w->storm, &w->darkness, &w->grey };
        for (int k = 0; k < 5; k++) bsc_clamp1(u[k], 0, 1);
        for (int v = 0; v < 3; v++) { bsc_clamp1(&w->cloud_tint[v], 0, 1.5f); bsc_clamp1(&w->grey_tone[v], 0, 2); }
        bsc_clamp1(&w->fog, 0, 0.05f);
    }
    bsc_clamp1(&c->star_count, 0, (float)max_stars);     bsc_clamp1(&c->cloud_count, 0, (float)max_clouds);
    bsc_clamp1(&c->sun_size, 0.5f, 12);                  bsc_clamp1(&c->moon_size, 0.5f, 12);
    bsc_clamp1(&c->cloud_scale, 0.2f, 3);                bsc_clamp1(&c->horizon_curve, 0.15f, 2);
    bsc_clamp1(&c->fog_base, 0, 0.05f);                  bsc_clamp1(&c->smoothing, 0.05f, 10);
    bsc_clamp1(&c->sun_tilt, -0.9f, 0.9f);               bsc_clamp1(&c->star_brightness, 0, 3);
    bsc_clamp1(&c->cloud_speed, 0, 20);
}

/* Load a file (parse into a copy of *c; only replace *c if the whole file is valid). Returns keys applied, or -1 with `err`. */
static inline int bigo_skycfg_load(BigoSkyConfig *c, const char *path, int max_stars, int max_clouds, char *err, size_t errsz) {
    ...
    BigoSkyConfig tmp = *c;
    int r = bigo_skycfg_parse(&tmp, text, err, errsz);
    if (r < 0) return -1;
    bigo_skycfg_sanitize(&tmp, max_stars, max_clouds);
    *c = tmp;
    return r;
}
#endif
