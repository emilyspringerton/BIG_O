/* bigo_sky.h -- procedural sky + weather rendering for BIG_O (GL 1.x immediate mode, no assets, no extra deps).
 * Driven by two numbers from the world sim: minute-of-day (0..1440, fractional ok) and weather (0 CLEAR 1 OVERCAST 2 RAIN 3 STORM).
 * Include AFTER the GL headers. Typical frame:
 *   bigo_sky_update(&sky, minute, weather, now_ms);
 *   glClearColor(sky.clear[0], sky.clear[1], sky.clear[2], 1); glClear(...);
 *   ... set projection + camera view (gluLookAt) ...
 *   bigo_sky_draw(&sky);                       // dome, sun/moon, stars, clouds (strips camera translation itself)
 *   bigo_sky_fog_on(&sky);  ...draw the world...  bigo_sky_fog_off();
 *   bigo_sky_draw_precip(&sky, cam_x, cam_y, cam_z, now_ms);   // rain streaks + lightning bolt
 *   bigo_sky_draw_grade(&sky, win_w, win_h);   // screen-space night tint + lightning flash (before the HUD)
 * Sun path: rises east 06:00, zenith 12:00, sets west 18:00 (slightly tilted south); moon is opposite. */
#ifndef BIGO_SKY_H
#define BIGO_SKY_H
#include <math.h>
#include <stdlib.h>
...
#define BSKY_STARS 320
#define BSKY_CLOUDS 44
#define BSKY_RAIN 700
#define BSKY_R 90.0f
#define BSKY_PI 3.14159265f
...
typedef struct {
    ...
    float star[BSKY_STARS][4];     /* x y z dir, brightness */
    float cloud[BSKY_CLOUDS][4];   /* azimuth, elevation, size, appear-threshold */
    float cover, rain, storm;      /* smoothed 0..1 weather intensities */
    float minute;                  /* current minute-of-day */
    int weather;
    unsigned int last_ms;
    float flash, flash_dx, flash_dz; unsigned int next_bolt_ms, bolt_seed; float bolt_age;
    /* derived each update */
...
static inline float bs_mix(float a, float b, float t) { return a + (b - a) * t; }
static inline float bs_smooth(float a, float b, float x) { float t = bs_clamp((x - a) / (b - a), 0, 1); return t * t * (3 - 2 * t); }
static inline void bs_mix3(float *o, const float *a, const float *b, float t) { for (int i = 0; i < 3; i++) o[i] = bs_mix(a[i], b[i], t); }
static inline unsigned int bs_rng(unsigned int *s) { *s ^= *s << 13; *s ^= *s >> 17; *s ^= *s << 5; return *s; }
static inline float bs_rf(unsigned int *s) { return (float)(bs_rng(s) & 0xFFFFFF) / 16777216.0f; }
static inline float bs_hash2(int x, int y) {
...
static inline float bs_vnoise(float x, float y) {
    int xi = (int)floorf(x), yi = (int)floorf(y); float fx = x - (float)xi, fy = y - (float)yi;
    fx = fx * fx * (3 - 2 * fx); fy = fy * fy * (3 - 2 * fy);
    return bs_mix(bs_mix(bs_hash2(xi, yi), bs_hash2(xi + 1, yi), fx), bs_mix(bs_hash2(xi, yi + 1), bs_hash2(xi + 1, yi + 1), fx), fy);
}

static inline void bigo_sky_init(BigoSky *s) {
    memset(s, 0, sizeof(*s));
    bigo_skycfg_defaults(&s->cfg);
    { const BscWeather *w = &s->cfg.weather[0]; s->dark = w->darkness; s->grey = w->grey; s->fogadd = w->fog; memcpy(s->tint, w->cloud_tint, 12); memcpy(s->grey_tone, w->grey_tone, 12); }
    unsigned int r = 0xB16B00B5u;
    for (int i = 0; i < BSKY_STARS; i++) {
        float y = 0.02f + bs_rf(&r) * 0.98f, a = bs_rf(&r) * 2 * BSKY_PI, rr = sqrtf(1 - y * y);
        s->star[i][0] = rr * cosf(a); s->star[i][1] = y; s->star[i][2] = rr * sinf(a);
        float b = bs_rf(&r); s->star[i][3] = 0.35f + 0.65f * b * b;
    }
    for (int i = 0; i < BSKY_CLOUDS; i++) {
        s->cloud[i][0] = bs_rf(&r) * 2 * BSKY_PI;
        s->cloud[i][1] = (5.0f + 78.0f * powf(bs_rf(&r), 1.05f)) * BSKY_PI / 180.0f;
        s->cloud[i][2] = 0.18f + 0.28f * bs_rf(&r) + 0.10f * (1.0f - s->cloud[i][1] / 1.4f);
        s->cloud[i][3] = (float)i / (float)BSKY_CLOUDS;     /* low index = appears first (clear-weather wisps) */
    }
    ...
    for (int y = 0; y < C; y++) for (int x = 0; x < C; x++) {
        ...
        for (int o = 0; o < 5; o++) { n += amp * bs_vnoise(u * f + 11.0f * (float)o, v * f + 5.0f * (float)o); amp *= 0.5f; f *= 2.0f; }
        float a = bs_smooth(0.20f, 0.62f, n * (1.15f - d) + (1.0f - d) * 0.45f - 0.22f);
        float shade = bs_clamp(0.72f + 0.28f * (-v * 0.8f + n * 0.6f), 0.55f, 1.0f);   /* v<0 is the top of the cloud */
...
/* Swap in a config (already sanitized or from bigo_skycfg_defaults). The sky eases to the new look rather than popping. */
static inline void bigo_sky_set_config(BigoSky *s, const BigoSkyConfig *c) { s->cfg = *c; bigo_skycfg_sanitize(&s->cfg, BSKY_STARS, BSKY_CLOUDS); }
/* Load a skybox file on top of the current config. Returns keys applied, or -1 with `err` (config left unchanged). */
static inline int bigo_sky_load_config(BigoSky *s, const char *path, char *err, size_t errsz) {
    BigoSkyConfig c = s->cfg; int r = bigo_skycfg_load(&c, path, BSKY_STARS, BSKY_CLOUDS, err, errsz);
    if (r >= 0) s->cfg = c;
    return r;
}

static inline void bigo_sky_update(BigoSky *s, float minute, int weather, unsigned int now_ms) {
    float dt = s->last_ms ? bs_clamp((float)(now_ms - s->last_ms) / 1000.0f, 0, 0.25f) : 1.0f;
    s->last_ms = now_ms; s->minute = minute; s->weather = weather;
    const BigoSkyConfig *cf = &s->cfg;
    int w = weather < 0 ? 0 : (weather > 3 ? 3 : weather);
    const BscWeather *wp = &cf->weather[w];
    ...
    float a = 2 * BSKY_PI * (minute - 360.0f) / 1440.0f;
    ...
    else if (e > 0.0f) { float t = e / 0.30f; bs_mix3(zd, zg, zen_day, t); bs_mix3(hd, hg, hor_day, t); }
    else if (e > -0.18f) { float t = (e + 0.18f) / 0.18f; bs_mix3(zd, zen_tw, zg, t); bs_mix3(hd, hor_tw, hg, t); }
    else { float t = bs_clamp((e + 0.42f) / 0.24f, 0, 1); bs_mix3(zd, zen_night, zen_tw, t * 0.35f); bs_mix3(hd, hor_night, hor_tw, t * 0.35f); }
    ...
    float gm = bs_clamp(s->grey, 0, 0.95f);
    for (int i = 0; i < 3; i++) {
        s->hor[i] = bs_mix(hd[i], grey_h[i], gm) * dark;
        s->zen[i] = bs_mix(zd[i], grey_z[i], gm) * bs_clamp(dark - 0.06f * s->storm, 0, 1);
    }
    s->night = bs_clamp(-e * 2.6f + 0.15f, 0, 1);

    /* horizon glow around the sun at sunrise/sunset, hidden by heavy cloud */
    s->glow_amt = bs_smooth(-0.22f, 0.02f, e) * (1.0f - bs_smooth(0.05f, 0.42f, e)) * (1.0f - 0.85f * s->cover);
    s->glow[0] = 1.0f; s->glow[1] = morning ? 0.52f : 0.42f; s->glow[2] = morning ? 0.36f : 0.20f;
    /* sunlight tint used for clouds */
    float warm = 1.0f - bs_smooth(0.05f, 0.45f, e);
    s->light[0] = 1.0f; s->light[1] = bs_mix(1.0f, 0.62f, warm); s->light[2] = bs_mix(1.0f, 0.42f, warm);
    float lit = bs_clamp(0.18f + 1.0f * bs_smooth(-0.25f, 0.30f, e), 0.10f, 1.0f) * (1.0f - 0.55f * s->cover) + 0.10f;
    for (int i = 0; i < 3; i++) s->light[i] *= lit * s->tint[i];
    { float ni[3] = { 0.10f, 0.13f, 0.26f }; float nt = s->night; for (int i = 0; i < 3; i++) s->light[i] = bs_mix(s->light[i], ni[i] * (1.0f - 0.4f * s->cover), nt * 0.8f); }
    ...
    if (s->flash > 0) s->flash = bs_clamp(s->flash - dt * 3.2f, 0, 1);
    ...
    if (s->storm > 0.6f) {
        ...
        if (now_ms >= s->next_bolt_ms) {
            ...
            float ang = bs_rf(&s->bolt_seed) * 2 * BSKY_PI; s->flash_dx = cosf(ang); s->flash_dz = sinf(ang);
            s->next_bolt_ms = now_ms + 3500 + (unsigned)(bs_rf(&s->bolt_seed) * 9000);
        }
    } else { s->next_bolt_ms = 0; }
}

static inline void bs_vert(const float *dir, float radius) { glVertex3f(dir[0] * radius, dir[1] * radius, dir[2] * radius); }

/* Strip the camera translation from the current modelview so the sky sits at infinity but still rotates with the view. */
static inline void bs_push_sky_matrix(void) {
    float m[16]; glGetFloatv(GL_MODELVIEW_MATRIX, m);
    m[12] = m[13] = m[14] = 0; glPushMatrix(); glLoadMatrixf(m);
}

static inline void bigo_sky_draw(BigoSky *s) {
    if (!s->ready) return;
    glMatrixMode(GL_MODELVIEW); bs_push_sky_matrix();
    ...
    const int R = 28, S = 48;
    for (int i = 0; i < R; i++) {
        float e0 = -0.35f + (BSKY_PI / 2 + 0.35f) * (float)i / R, e1 = -0.35f + (BSKY_PI / 2 + 0.35f) * (float)(i + 1) / R;
        glBegin(GL_QUAD_STRIP);
        for (int j = 0; j <= S; j++) {
            float az = 2 * BSKY_PI * (float)j / S;
            for (int k = 0; k < 2; k++) {
                float el = k ? e1 : e0, y = sinf(el), rr = cosf(el);
                float d[3] = { rr * cosf(az), y, rr * sinf(az) };
                float h = bs_clamp(y, 0, 1);
                float t = powf(h, s->cfg.horizon_curve);                                  /* fast rise off the horizon, long gentle zenith */
                float c[3]; bs_mix3(c, s->hor, s->zen, t);
                float dot = d[0] * s->sun[0] + d[1] * s->sun[1] + d[2] * s->sun[2];
                float g = powf(bs_clamp(dot, 0, 1), 5.0f) * s->glow_amt * (1.0f - 0.6f * t) + powf(bs_clamp(dot, 0, 1), 60.0f) * s->glow_amt * 0.5f;
                for (int q = 0; q < 3; q++) c[q] = bs_clamp(c[q] + s->glow[q] * g * 0.85f, 0, 1);
                if (y < 0) for (int q = 0; q < 3; q++) c[q] = bs_mix(c[q], s->hor[q] * 0.75f, bs_clamp(-y * 2.5f, 0, 1));  /* ground-side haze */
                glColor3f(c[0], c[1], c[2]); glVertex3f(d[0] * BSKY_R, d[1] * BSKY_R, d[2] * BSKY_R);
            }
        }
        glEnd();
    }
    ...
    float star_a = bs_clamp(s->night * 1.25f - 0.10f, 0, 1) * (1.0f - bs_clamp(s->cover * 1.3f, 0, 1)) * s->cfg.star_brightness;
    ...
    if (star_a > 0.01f) {
        ...
        for (int pass = 0; pass < 2; pass++) {
            ...
            for (int i = 0; i < nstars; i++) {
                ...
                glVertex3f(s->star[i][0] * BSKY_R * 0.98f, s->star[i][1] * BSKY_R * 0.98f, s->star[i][2] * BSKY_R * 0.98f);
            }
            glEnd();
        }
    }
    ...
    for (int body = 0; body < 2; body++) {
        ...
        float vis = body ? bs_smooth(-0.05f, -0.25f, s->sun_e) : bs_smooth(-0.14f, 0.02f, s->sun_e);
        vis *= 1.0f - 0.92f * bs_clamp(s->cover * 1.05f, 0, 1);
        ...
        float c0 = body ? 0.62f : 1.0f, c1 = body ? 0.72f : (0.82f + 0.15f * bs_smooth(0.05f, 0.4f, s->sun_e)), c2 = body ? 1.0f : (0.55f + 0.4f * bs_smooth(0.0f, 0.5f, s->sun_e));
        ...
        for (int layer = 0; layer < 2; layer++) {
            ...
            for (int q = 0; q < 4; q++) {
                ...
                glVertex3f(d[0] * BSKY_R * 0.95f + (right[0] * sx + u2[0] * sy) * sz, d[1] * BSKY_R * 0.95f + (right[1] * sx + u2[1] * sy) * sz, d[2] * BSKY_R * 0.95f + (right[2] * sx + u2[2] * sy) * sz);
            }
            glEnd();
        }
        ...
        glVertex3f(d[0] * BSKY_R * 0.94f, d[1] * BSKY_R * 0.94f, d[2] * BSKY_R * 0.94f);
        for (int q = 0; q <= 24; q++) {
            float a = 2 * BSKY_PI * (float)q / 24;
            glVertex3f(d[0] * BSKY_R * 0.94f + (right[0] * cosf(a) + u2[0] * sinf(a)) * sz_disc, d[1] * BSKY_R * 0.94f + (right[1] * cosf(a) + u2[1] * sinf(a)) * sz_disc, d[2] * BSKY_R * 0.94f + (right[2] * cosf(a) + u2[2] * sinf(a)) * sz_disc);
        }
        glEnd();
    ...
    float thick = bs_clamp(0.30f + s->cover * 0.75f, 0, 1);
    for (int i = 0; i < nclouds; i++) {
        float thr = (float)i / (float)(nclouds > 0 ? nclouds : 1);
        float appear = bs_clamp((s->cover * 1.15f - thr * 0.95f) * 4.0f, 0, 1);
        ...
        float rl = sqrtf(right[0] * right[0] + right[1] * right[1] + right[2] * right[2]); for (int q = 0; q < 3; q++) right[q] /= rl;
        float u2[3] = { d[1] * right[2] - d[2] * right[1], d[2] * right[0] - d[0] * right[2], d[0] * right[1] - d[1] * right[0] };
        float sz = s->cloud[i][2] * s->cfg.cloud_scale * BSKY_R * (0.85f + 1.5f * s->cover * s->cover), sy = sz * (0.50f + 0.34f * s->cover);
        /* underside darkens with cover/storm; the sun side of the sky catches warm light */
        float sundot = bs_clamp(d[0] * s->sun[0] + d[1] * s->sun[1] + d[2] * s->sun[2], 0, 1);
        float rim = powf(sundot, 6.0f) * (1.0f - bs_smooth(0.2f, 0.5f, s->sun_e)) * (1.0f - s->cover * 0.6f);
        float cr = s->light[0] + rim * 0.55f, cg = s->light[1] + rim * 0.25f, cb = s->light[2] + rim * 0.05f;
        float haze = bs_clamp(1.0f - el * 2.2f, 0, 1) * 0.35f;   /* low clouds melt into the horizon colour */
        cr = bs_mix(cr, s->hor[0], haze); cg = bs_mix(cg, s->hor[1], haze); cb = bs_mix(cb, s->hor[2], haze);
        float flip = (i & 1) ? -1.f : 1.f;
        glColor4f(bs_clamp(cr, 0, 1), bs_clamp(cg, 0, 1), bs_clamp(cb, 0, 1), appear * thick * (0.55f + 0.4f * s->cover));
        glBegin(GL_QUADS);
        for (int q = 0; q < 4; q++) {
            float qx = (q == 0 || q == 3) ? -1.f : 1.f, qy = (q < 2) ? -1.f : 1.f;
            glTexCoord2f(qx * 0.5f * flip + 0.5f, qy * 0.5f + 0.5f);
            glVertex3f(d[0] * BSKY_R * 0.9f + right[0] * qx * sz + u2[0] * qy * sy, d[1] * BSKY_R * 0.9f + right[1] * qx * sz + u2[1] * qy * sy, d[2] * BSKY_R * 0.9f + right[2] * qx * sz + u2[2] * qy * sy);
        }
        glEnd();
    }
    glDisable(GL_TEXTURE_2D);
...
static inline void bigo_sky_fog_on(const BigoSky *s) {
...
static inline void bigo_sky_fog_off(void) { glDisable(GL_FOG); }

/* Rain streaks around the camera (world space) and the storm's lightning bolt. Call after the world, before the HUD. */
static inline void bigo_sky_draw_precip(BigoSky *s, float cx, float cy, float cz, unsigned int now_ms) {
    if (!s->ready) return;
    glDisable(GL_TEXTURE_2D); glEnable(GL_BLEND); glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    if (s->rain > 0.03f) {
        int n = (int)(BSKY_RAIN * s->rain);
        float t = (float)now_ms * 0.001f, wind = 0.18f + 0.30f * s->storm, speed = 22.0f + 10.0f * s->storm;
        glLineWidth(1.0f);
        glBegin(GL_LINES);
        for (int i = 0; i < n; i++) {
            float hx = bs_hash2(i, 1), hz = bs_hash2(i, 2), hp = bs_hash2(i, 3), hl = bs_hash2(i, 4);
    ...
    if (s->storm > 0.6f && s->bolt_age >= 0 && s->bolt_age < 0.28f) {
        ...
        for (int pass = 0; pass < 2; pass++) {
            ...
            for (int k = 0; k < 11; k++) {
                glVertex3f(x, y, z);
                x += (bs_rf(&rr) - 0.5f) * 7.0f; z += (bs_rf(&rr) - 0.5f) * 7.0f; y -= 4.0f;
            }
            glEnd();
        }
    }
...
/* Screen-space grading: blue night tint, storm gloom, lightning flash. Draws in ortho over the scene; HUD comes after. */
static inline void bigo_sky_draw_grade(const BigoSky *s, int win_w, int win_h) {
