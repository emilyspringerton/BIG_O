...
if [ "$BUILD_SIM" = 1 ]; then
  ...
  gcc $CFLAGS_BASE -g -fsanitize=address,undefined -fno-sanitize-recover=all \
      apps/bigo_sim/main.c core/sim.c core/witness_rules.c -o build/bigo_sim_asan
  gcc $CFLAGS_BASE -O2 apps/bigo_sim/main.c core/sim.c core/witness_rules.c -o build/bigo_sim
...
if [[ " $* " == *" --windows "* ]]; then
  ...
  if [ "$BUILD_SIM" = 1 ]; then
    x86_64-w64-mingw32-gcc $CFLAGS_BASE -O2 apps/bigo_sim/main.c core/sim.c core/witness_rules.c -o build/bigo_sim.exe
